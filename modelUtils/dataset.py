...
def generateOwndata(grid_size):
# Define the size of the checkerboard (4x4 in this case)
    grid_size = grid_size
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            # Calculate the XOR-like label
            label = 1 if (i % 2 == 0) != (j % 2 == 0) else 0
            
            # Generate random data points within a small range
            x = np.random.uniform(i / grid_size, (i + 1) / grid_size)
            y = np.random.uniform(j / grid_size, (j + 1) / grid_size)
...
class DatasetCheckerboard4X4_Own(Dataset):
    ...
    def __init__(self):
        ...
        for i in range(63):
            Data,Labels=generateOwndata(grid_size)

            traindata=np.append(Data,traindata)
            trainlabel=np.append(Labels,trainlabel)

        self.trainData = traindata.reshape(-1, 2)
        self.trainLabels = trainlabel.reshape(-1, 1)
        scaler = preprocessing.StandardScaler().fit(self.trainData)
        self.trainData = scaler.transform(self.trainData)
        ...
        for i in range(63):
            ...
            testdata=np.append(Data,testdata)
            testlabel=np.append(Labels,testlabel)

        self.testData = testdata.reshape(-1, 2)
        self.testLabels = testlabel.reshape(-1, 1)
        self.testData = scaler.transform(self.testData)
